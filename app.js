...
app.get('/api/courses/stats', (req, res) => {
    try {
        ...
        const byStatus = statuses.reduce((stats, status) => {
            stats[status] = courses.filter(course => course.status === status).length;
            return stats;
        }, {});
        ...
        res.status(500).json({
            success: false,
            error: `Failed to retrieve course statistics: ${error.message}`
        });
    }
});
